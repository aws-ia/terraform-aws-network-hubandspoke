...
EOF
  ...
  validation {
    error_message = "Only valid key values for items in var.spoke_vpcs.vpc_information: \"vpc_id\", \"transit_gateway_attachment_id\"."
    condition = alltrue(
      [
        for vpc in try(var.spoke_vpcs.vpc_information, {}) : length(setsubtract(keys(vpc), [
          "vpc_id",
