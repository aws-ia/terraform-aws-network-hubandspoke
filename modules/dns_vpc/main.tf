...
resource "aws_subnet" "vpc_tgw_subnets" {
  ...
  tags = {
    Name = "dns_vpc-tgw_subnet-${var.identifier}-${count.index + 1}"
  }
}
...
resource "aws_subnet" "vpc_dns_subnets" {
  ...
  tags = {
    Name = "dns_vpc-dns_subnet-${var.identifier}-${count.index + 1}"
  }
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw_attachment" {
  ...
  tags = {
    Name = "tgw_dns_attachment-${var.identifier}"
  }
}
...
resource "aws_route_table" "vpc_tgw_subnet_rt" {
  ...
  tags = {
    Name = "dns_vpc-tgw_subnet-rt-${var.identifier}-${count.index + 1}"
  }
}
...
# DNS Subnet Route Tables
resource "aws_route_table" "vpc_dns_subnet_rt" {
  count  = var.number_azs
  vpc_id = aws_vpc.vpc.id

  tags = {
    Name = "dns_vpc-dns_subnet-rt-${var.identifier}-${count.index + 1}"
  }
}

resource "aws_route_table_association" "vpc_dns_rt_assoc" {
  count          = var.number_azs
...
# VPC ROUTES
# Traffic from DNS subnets to TGW
...
resource "aws_cloudwatch_log_group" "flowlogs_lg" {
  count             = var.enable_logging ? 1 : 0
  name              = "dns_vpc-lg-vpc-flowlogs-${var.identifier}"
  retention_in_days = 7
